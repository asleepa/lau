...
#define makewhite(g,x)	\
  (x->marked = cast_byte((x->marked & ~maskcolors) | lauC_white(g)))
...
static void atomic (lau_State *L);
static void entersweep (lau_State *L);
...
static l_mem objsize (GCObject *o) {
  ...
  switch (o->tt) {
    ...
    case LAU_VPROTO: {
      res = lauF_protosize(gco2p(o));
      break;
    }
    case LAU_VTHREAD: {
      res = lauE_threadsize(gco2th(o));
      break;
    }
    case LAU_VSHRSTR: {
      TString *ts = gco2ts(o);
    ...
    case LAU_VLNGSTR: {
      TString *ts = gco2ts(o);
      res = lauS_sizelngstr(ts->u.lnglen, ts->shrlen);
      break;
    }
    case LAU_VUPVAL: {
      res = sizeof(UpVal);
...
static GCObject **getgclist (GCObject *o) {
  switch (o->tt) {
    case LAU_VTABLE: return &gco2t(o)->gclist;
    case LAU_VLCL: return &gco2lcl(o)->gclist;
    case LAU_VCCL: return &gco2ccl(o)->gclist;
    case LAU_VTHREAD: return &gco2th(o)->gclist;
    case LAU_VPROTO: return &gco2p(o)->gclist;
    case LAU_VUSERDATA: {
      Udata *u = gco2u(o);
      lau_assert(u->nuvalue > 0);
      return &u->gclist;
    }
...
static void linkgclist_ (GCObject *o, GCObject **pnext, GCObject **list) {
  lau_assert(!isgray(o));  /* cannot be in a gray list */
  *pnext = *list;
  *list = o;
...
static int iscleared (global_State *g, const GCObject *o) {
  ...
  else if (novariant(o->tt) == LAU_TSTRING) {
...
** white from dead.)
*/
void lauC_barrier_ (lau_State *L, GCObject *o, GCObject *v) {
  global_State *g = G(L);
  lau_assert(isblack(o) && iswhite(v) && !isdead(g, v) && !isdead(g, o));
  if (keepinvariant(g)) {  /* must keep invariant? */
    reallymarkobject(g, v);  /* restore invariant */
    if (isold(o)) {
      lau_assert(!isold(v));  /* white object could not be old */
      setage(v, G_OLD0);  /* restore generational invariant */
    }
  }
  else {  /* sweep phase */
    lau_assert(issweepphase(g));
...
*/
void lauC_barrierback_ (lau_State *L, GCObject *o) {
  global_State *g = G(L);
  lau_assert(isblack(o) && !isdead(g, o));
  lau_assert((g->gckind != KGC_GENMINOR)
...
void lauC_fix (lau_State *L, GCObject *o) {
  global_State *g = G(L);
  lau_assert(g->allgc == o);  /* object must be 1st in 'allgc' list! */
...
*/
GCObject *lauC_newobjdt (lau_State *L, lu_byte tt, size_t sz, size_t offset) {
  global_State *g = G(L);
  char *p = cast_charp(lauM_newobject(L, novariant(tt), sz));
  GCObject *o = cast(GCObject *, p + offset);
  o->marked = lauC_white(g);
  o->tt = tt;
  o->next = g->allgc;
  g->allgc = o;
  return o;
...
*/
GCObject *lauC_newobj (lau_State *L, lu_byte tt, size_t sz) {
  return lauC_newobjdt(L, tt, sz, 0);
}
...
static void reallymarkobject (global_State *g, GCObject *o) {
  ...
  switch (o->tt) {
    ...
    case LAU_VLCL: case LAU_VCCL: case LAU_VTABLE:
    case LAU_VTHREAD: case LAU_VPROTO: {
      linkobjgclist(o, g->gray);  /* to be visited later */
      break;
    }
    default: lau_assert(0); break;
  }
}
...
static void remarkupvals (global_State *g) {
  ...
  while ((thread = *p) != NULL) {
    if (!iswhite(thread) && thread->openupval != NULL)
      ...
      UpVal *uv;
      lau_assert(!isold(thread) || thread->openupval == NULL);
      *p = thread->twups;  /* remove thread from the list */
      thread->twups = thread;  /* mark that it is out of list */
      for (uv = thread->openupval; uv != NULL; uv = uv->u.open.next) {
        lau_assert(getage(uv) <= getage(thread));
        if (!iswhite(uv)) {  /* upvalue already visited? */
          lau_assert(upisopen(uv) && isgray(uv));
...
/*
** Traverse a Lau closure, marking its prototype and its upvalues.
...
*/
static l_mem traversethread (global_State *g, lau_State *th) {
  UpVal *uv;
  StkId o = th->stack.p;
  ...
  lau_assert(g->gcstate == GCSatomic ||
  ...
  if (g->gcstate == GCSatomic) {  /* final traversal? */
    if (!g->gcemergency)
      lauD_shrinkstack(th); /* do not change stack in emergency cycle */
...
static l_mem propagatemark (global_State *g) {
  ...
  switch (o->tt) {
    case LAU_VTABLE: return traversetable(g, gco2t(o));
    case LAU_VUSERDATA: return traverseudata(g, gco2u(o));
    case LAU_VLCL: return traverseLclosure(g, gco2lcl(o));
    case LAU_VCCL: return traverseCclosure(g, gco2ccl(o));
    case LAU_VPROTO: return traverseproto(g, gco2p(o));
    case LAU_VTHREAD: return traversethread(g, gco2th(o));
    default: lau_assert(0); return 0;
  }
}
  ...
  for (; l != f; l = gco2t(l)->gclist) {
    ...
    for (i = 0; i < asize; i++) {
      ...
      if (iscleared(g, o))  /* value was collected? */
        *getArrTag(h, i) = LAU_VEMPTY;  /* remove entry */
    }
    for (n = gnode(h, 0); n < limit; n++) {
...
static void freeupval (lau_State *L, UpVal *uv) {
  if (upisopen(uv))
    lauF_unlinkupval(uv);
  lauM_free(L, uv);
}
...
static void freeobj (lau_State *L, GCObject *o) {
  assert_code(l_mem newmem = gettotalbytes(G(L)) - objsize(o));
  switch (o->tt) {
    case LAU_VPROTO:
      lauF_freeproto(L, gco2p(o));
      break;
    case LAU_VUPVAL:
    ...
    case LAU_VLCL: {
      LClosure *cl = gco2lcl(o);
      lauM_freemem(L, cl, sizeLclosure(cl->nupvalues));
      break;
    }
    case LAU_VCCL: {
      CClosure *cl = gco2ccl(o);
      lauM_freemem(L, cl, sizeCclosure(cl->nupvalues));
      break;
    }
    case LAU_VTABLE:
      lauH_free(L, gco2t(o));
      break;
    case LAU_VTHREAD:
      lauE_freethread(L, gco2th(o));
      break;
    case LAU_VUSERDATA: {
      Udata *u = gco2u(o);
      lauM_freemem(L, o, sizeudata(u->nuvalue, u->len));
      break;
    }
    case LAU_VSHRSTR: {
      TString *ts = gco2ts(o);
      lauS_remove(L, ts);  /* remove it from hash table */
      lauM_freemem(L, ts, sizestrshr(cast_uint(ts->shrlen)));
      break;
    }
    case LAU_VLNGSTR: {
      TString *ts = gco2ts(o);
      ...
      lauM_freemem(L, ts, lauS_sizelngstr(ts->u.lnglen, ts->shrlen));
      break;
    }
    default: lau_assert(0);
  }
  lau_assert(gettotalbytes(G(L)) == newmem);
}
...
*/
static GCObject **sweeplist (lau_State *L, GCObject **p, l_mem countin) {
  global_State *g = G(L);
  int ow = otherwhite(g);
  int white = lauC_white(g);  /* current white */
...
*/
static GCObject **sweeptolive (lau_State *L, GCObject **p) {
  GCObject **old = p;
  do {
    p = sweeplist(L, p, 1);
  } while (p == old);
...
*/
static void checkSizes (lau_State *L, global_State *g) {
  if (!g->gcemergency) {
    if (g->strt.nuse < g->strt.size / 4)  /* string table too big? */
      lauS_resize(L, g->strt.size / 2);
  }
}
...
static void dothecall (lau_State *L, void *ud) {
  UNUSED(ud);
  lauD_callnoyield(L, L->top.p - 2, 0);
}


static void GCTM (lau_State *L) {
  global_State *g = G(L);
  const TValue *tm;
  TValue v;
  lau_assert(!g->gcemergency);
  setgcovalue(L, &v, udata2finalize(g));
  tm = lauT_gettmbyobj(L, &v, TM_GC);
  if (!notm(tm)) {  /* is there a finalizer? */
    ...
    status = lauD_pcall(L, dothecall, NULL, savestack(L, L->top.p - 2), 0);
    L->ci->callstatus &= ~CIST_FIN;  /* not running a finalizer anymore */
    L->allowhook = oldah;  /* restore hooks */
    g->gcstp = oldgcstp;  /* restore state */
    if (l_unlikely(status != LAU_OK)) {  /* error while running __gc? */
      lauE_warnerror(L, "__gc");
...
*/
static void callallpendingfinalizers (lau_State *L) {
  global_State *g = G(L);
  while (g->tobefnz)
    GCTM(L);
}
...
*/
void lauC_checkfinalizer (lau_State *L, GCObject *o, Table *mt) {
  global_State *g = G(L);
...
*/
static void sweep2old (lau_State *L, GCObject **p) {
  GCObject *curr;
  global_State *g = G(L);
  while ((curr = *p) != NULL) {
    if (iswhite(curr)) {  /* is 'curr' dead? */
      lau_assert(isdead(g, curr));
      *p = curr->next;  /* remove 'curr' from list */
      freeobj(L, curr);  /* erase 'curr' */
    }
    else {  /* all surviving objects become old */
      setage(curr, G_OLD);
      if (curr->tt == LAU_VTHREAD) {  /* threads must be watched */
        lau_State *th = gco2th(curr);
        linkgclist(th, g->grayagain);  /* insert into 'grayagain' list */
      }
      else if (curr->tt == LAU_VUPVAL && upisopen(gco2upv(curr)))
...
*/
static GCObject **sweepgen (lau_State *L, global_State *g, GCObject **p,
  ...
  l_mem addedold = 0;
  int white = lauC_white(g);
  GCObject *curr;
  while ((curr = *p) != limit) {
    if (iswhite(curr)) {  /* is 'curr' dead? */
      lau_assert(!isold(curr) && isdead(g, curr));
      *p = curr->next;  /* remove 'curr' from list */
      freeobj(L, curr);  /* erase 'curr' */
    }
      ...
      if (age == G_NEW) {  /* new objects go back to white */
        ...
        lau_assert(age != G_OLD1);  /* advanced in 'markold' */
...
static GCObject **correctgraylist (GCObject **p) {
  ...
  while ((curr = *p) != NULL) {
    ...
    if (iswhite(curr))
      ...
      lau_assert(isgray(curr));
      nw2black(curr);  /* make it black, for next barrier */
      setage(curr, G_TOUCHED2);
      goto remain;  /* keep it in the list and go to next element */
    }
    else if (curr->tt == LAU_VTHREAD) {
      lau_assert(isgray(curr));
      goto remain;  /* keep non-white threads on the list */
    }
    else {  /* everything else is removed */
      lau_assert(isold(curr));  /* young objects should be white here */
...
*/
static void finishgencycle (lau_State *L, global_State *g) {
  correctgraylists(g);
  checkSizes(L, g);
  g->gcstate = GCSpropagate;  /* skip restart */
  if (g->tobefnz != NULL && !g->gcemergency && lauD_checkminstack(L))
...
*/
static void minor2inc (lau_State *L, global_State *g, lu_byte kind) {
  ...
  lauE_setdebt(g, applygcparam(g, STEPSIZE, 100));
}
...
*/
static void youngcollection (lau_State *L, global_State *g) {
  l_mem addedold1 = 0;
  l_mem marked = g->GCmarked;  /* preserve 'g->GCmarked' */
  GCObject **psurvival;  /* to point to first non-dead survival object */
  GCObject *dummy;  /* dummy out parameter to 'sweepgen' */
  lau_assert(g->gcstate == GCSpropagate);
...
*/
static void atomic2gen (lau_State *L, global_State *g) {
  cleargraylists(g);
...
static void setminordebt (global_State *g) {
  lauE_setdebt(g, applygcparam(g, MINORMUL, g->GCmajorminor));
}
...
** collection.
*/
static void entergen (lau_State *L, global_State *g) {
  lauC_runtilstate(L, GCSpause, 1);  /* prepare to start a new cycle */
  lauC_runtilstate(L, GCSpropagate, 1);  /* start new cycle */
...
*/
void lauC_changemode (lau_State *L, int newmode) {
  global_State *g = G(L);
  ...
  if (newmode != g->gckind) {  /* does it need to change? */
    if (newmode == KGC_INC)  /* entering incremental mode? */
      ...
      lau_assert(newmode == KGC_GENMINOR);
      entergen(L, g);
    }
  }
}
...
*/
static void fullgen (lau_State *L, global_State *g) {
...
*/
static int checkmajorminor (lau_State *L, global_State *g) {
...
** 'limit'.
*/
static void deletelist (lau_State *L, GCObject *p, GCObject *limit) {
  while (p != limit) {
...
/*
** Call all finalizers of the objects in the given Lau state, and
** then free all objects, except for the main thread.
*/
void lauC_freeallobjects (lau_State *L) {
  global_State *g = G(L);
  g->gcstp = GCSTPCLS;  /* no extra finalizers after here */
  lauC_changemode(L, KGC_INC);
  separatetobefnz(g, 1);  /* separate all objects with finalizers */
  lau_assert(g->finobj == NULL);
  callallpendingfinalizers(L);
  deletelist(L, g->allgc, obj2gco(mainthread(g)));
  lau_assert(g->finobj == NULL);  /* no new finalizers */
  deletelist(L, g->fixedgc, NULL);  /* collect fixed objects */
  lau_assert(g->strt.nuse == 0);
}
...
static void atomic (lau_State *L) {
  ...
  g->grayagain = NULL;
  lau_assert(g->ephemeron == NULL && g->weak == NULL);
  lau_assert(!iswhite(mainthread(g)));
  g->gcstate = GCSatomic;
...
*/
static void sweepstep (lau_State *L, global_State *g,
...
static l_mem singlestep (lau_State *L, int fast) {
  global_State *g = G(L);
  l_mem stepresult;
  lau_assert(!g->gcstopem);  /* collector is not reentrant */
  ...
  switch (g->gcstate) {
    ...
    case GCScallfin: {  /* call finalizers */
      if (g->tobefnz && !g->gcemergency && lauD_checkminstack(L)) {
...
** here is always true.)
*/
void lauC_runtilstate (lau_State *L, int state, int fast) {
  global_State *g = G(L);
  lau_assert(g->gckind == KGC_INC);
...
*/
static void incstep (lau_State *L, global_State *g) {
  ...
  if (g->gcstate == GCSpause)
    ...
    lauE_setdebt(g, stepsize);
}


#if !defined(laui_tracegc)
#define laui_tracegc(L,f)		((void)0)
#endif
...
*/
void lauC_step (lau_State *L) {
  global_State *g = G(L);
  lau_assert(!g->gcemergency);
  if (!gcrunning(g)) {  /* not running? */
    if (g->gcstp & GCSTPUSR)  /* stopped by the user? */
      lauE_setdebt(g, 20000);
  }
  else {
    laui_tracegc(L, 1);  /* for internal debugging */
    switch (g->gckind) {
    ...
    laui_tracegc(L, 0);  /* for internal debugging */
  }
}
...
*/
static void fullinc (lau_State *L, global_State *g) {
  if (keepinvariant(g))  /* black objects? */
    entersweep(L); /* sweep everything to turn them back to white */
  /* finish any pending sweep phase to start a new cycle */
  lauC_runtilstate(L, GCSpause, 1);
  lauC_runtilstate(L, GCScallfin, 1);  /* run up to finalizers */
  lauC_runtilstate(L, GCSpause, 1);  /* finish collection */
  setpause(g);
}
...
*/
void lauC_fullgc (lau_State *L, int isemergency) {
  global_State *g = G(L);
  lau_assert(!g->gcemergency);
