...
typedef struct {
  ...
  lau_Unsigned nstr;  /* counter for counting saved strings */
} DumpState;
...
static void dumpAlign (DumpState *D, unsigned align) {
  ...
  if (padding < align) {  /* padding == align means no padding */
    static lau_Integer paddingContent = 0;
    lau_assert(align <= sizeof(lau_Integer));
    dumpBlock(D, &paddingContent, padding);
  }
  lau_assert(D->offset % align == 0);
}
...
*/
#define DIBS    ((l_numbits(lau_Unsigned) + 6) / 7)

/*
** Dumps an unsigned integer using the MSB Varint encoding
*/
static void dumpVarint (DumpState *D, lau_Unsigned x) {
  lu_byte buff[DIBS];
  unsigned n = 1;
...
static void dumpSize (DumpState *D, size_t sz) {
  dumpVarint(D, cast(lau_Unsigned, sz));
}
...
static void dumpNumber (DumpState *D, lau_Number x) {
  dumpVar(D, x);
}
...
** (0 => 0; -1 => 1; 1 => 2; -2 => 3; 2 => 4; ...)
*/
static void dumpInteger (DumpState *D, lau_Integer x) {
  lau_Unsigned cx = (x >= 0) ? 2u * l_castS2U(x)
                             : (2u * ~l_castS2U(x)) + 1;
  dumpVarint(D, cx);
}
...
static void dumpString (DumpState *D, TString *ts) {
  if (ts == NULL) {
    ...
    TValue idx;
    int tag = lauH_getstr(D->h, ts, &idx);
    if (!tagisempty(tag)) {  /* string already saved? */
      ...
      lauH_set(D->L, D->h, &key, &value);  /* h[ts] = nstr */
  ...
  for (i = 0; i < n; i++) {
    ...
    switch (tt) {
      ...
      case LAU_VLNGSTR:
        ...
        break;
      default:
        lau_assert(tt == LAU_VNIL || tt == LAU_VFALSE || tt == LAU_VTRUE);
    }
  }
}
...
static void dumpHeader (DumpState *D) {
  dumpLiteral(D, LAU_SIGNATURE);
  dumpByte(D, LAUC_VERSION);
  dumpByte(D, LAUC_FORMAT);
  dumpLiteral(D, LAUC_DATA);
  dumpNumInfo(D, int, LAUC_INT);
  dumpNumInfo(D, Instruction, LAUC_INST);
  dumpNumInfo(D, lau_Integer, LAUC_INT);
  dumpNumInfo(D, lau_Number, LAUC_NUM);
}


/*
** dump Lau function as precompiled chunk
*/
int lauU_dump (lau_State *L, const Proto *f, lau_Writer w, void *data,
               int strip) {
  DumpState D;
  D.h = lauH_new(L);  /* aux. table to keep strings already dumped */
