...
static int str_reverse (lau_State *L) {
  size_t l, i;
  lauL_Buffer b;
  const char *s = lauL_checklstring(L, 1, &l);
...
/*
** MAX_SIZE is limited both by size_t and lau_Integer.
** When x <= MAX_SIZE, x can be safely cast to size_t or lau_Integer.
*/
static int str_rep (lau_State *L) {
  size_t len, lsep;
  const char *s = lauL_checklstring(L, 1, &len);
  lau_Integer n = lauL_checkinteger(L, 2);
  const char *sep = lauL_optlstring(L, 3, "", &lsep);
  if (n <= 0 || (len | lsep) == 0)
    lau_pushliteral(L, "");  /* no repetitions or both strings empty */
  else if (l_unlikely(len > MAX_SIZE - lsep ||
               cast_st2S(len + lsep) > cast_st2S(MAX_SIZE) / n))
    return lauL_error(L, "resulting string too large");
  else {
    size_t totallen = (cast_sizet(n) * (len + lsep)) - lsep;
    lauL_Buffer b;
    char *p = lauL_buffinitsize(L, &b, totallen);
    ...
    lauL_pushresultsize(&b, totallen);
  }
  return 1;
}


static int str_byte (lau_State *L) {
  size_t l;
  const char *s = lauL_checklstring(L, 1, &l);
  lau_Integer pi = lauL_optinteger(L, 2, 1);
  ...
  for (i=0; i<n; i++)
    lau_pushinteger(L, cast_uchar(s[posi + cast_uint(i) - 1]));
  return n;
}


static int str_char (lau_State *L) {
  int n = lau_gettop(L);  /* number of arguments */
  int i;
  lauL_Buffer b;
  char *p = lauL_buffinitsize(L, &b, cast_uint(n));
  for (i=1; i<=n; i++) {
    lau_Unsigned c = (lau_Unsigned)lauL_checkinteger(L, i);
    lauL_argcheck(L, c <= (lau_Unsigned)UCHAR_MAX, i, "value out of range");
    p[i - 1] = cast_char(cast_uchar(c));
  }
  lauL_pushresultsize(&b, cast_uint(n));
  return 1;
}
...
** after the call to 'lau_dump', to ensure that the function is on the
** top of the stack when 'lau_dump' is called. ('lauL_buffinit' might
** push stuff.)
*/
struct str_Writer {
...
static int writer (lau_State *L, const void *b, size_t size, void *ud) {
  struct str_Writer *state = (struct str_Writer *)ud;
  if (!state->init) {
    state->init = 1;
    lauL_buffinit(L, &state->B);
  }
  if (b == NULL) {  /* finishing dump? */
    lauL_pushresult(&state->B);  /* push result */
    lau_replace(L, 1);  /* move it to reserved slot */
  }
  else
    lauL_addlstring(&state->B, (const char *)b, size);
  return 0;
}


static int str_dump (lau_State *L) {
  struct str_Writer state;
  int strip = lau_toboolean(L, 2);
  lauL_argcheck(L, lau_type(L, 1) == LAU_TFUNCTION && !lau_iscfunction(L, 1),
                   1, "Lau function expected");
  /* ensure function is on the top of the stack and vacate slot 1 */
  lau_pushvalue(L, 1);
  state.init = 0;
  lau_dump(L, writer, &state, strip);
  lau_settop(L, 1);  /* leave final result on top */
  return 1;
}
...
static int gmatch_aux (lau_State *L) {
  GMatchState *gm = (GMatchState *)lau_touserdata(L, lau_upvalueindex(3));
  const char *src;
  gm->ms.L = L;
...
static int gmatch (lau_State *L) {
  size_t ls, lp;
  const char *s = lauL_checklstring(L, 1, &ls);
  const char *p = lauL_checklstring(L, 2, &lp);
  size_t init = posrelatI(lauL_optinteger(L, 3, 1), ls) - 1;
  GMatchState *gm;
  lau_settop(L, 2);  /* keep strings on closure to avoid being collected */
  gm = (GMatchState *)lau_newuserdatauv(L, sizeof(GMatchState), 0);
  if (init > ls)  /* start after string's end? */
    init = ls + 1;  /* avoid overflows in 's + init' */
  prepstate(&gm->ms, L, s, ls, p, lp);
  gm->src = s + init; gm->p = p; gm->lastmatch = NULL;
  lau_pushcclosure(L, gmatch_aux, 3);
  return 1;
}
...
#if !defined(lau_number2strx)	/* { */

/*
** Hexadecimal floating-point formatter
*/

#define SIZELENMOD	(sizeof(LAU_NUMBER_FRMLEN)/sizeof(char))
...
*/
static lau_Number adddigit (char *buff, unsigned n, lau_Number x) {
  lau_Number dd = l_mathop(floor)(x);  /* get integer part from 'x' */
  int d = (int)dd;
...
static int num2straux (char *buff, unsigned sz, lau_Number x) {
  /* if 'inf' or 'NaN', format it like '%g' */
  if (x != x || x == (lau_Number)HUGE_VAL || x == -(lau_Number)HUGE_VAL)
    return l_sprintf(buff, sz, LAU_NUMBER_FMT, (LAUI_UACNUMBER)x);
  else if (x == 0) {  /* can be -0... */
    /* create "0" or "-0" followed by exponent */
    return l_sprintf(buff, sz, LAU_NUMBER_FMT "x0p+0", (LAUI_UACNUMBER)x);
  }
  else {
    int e;
    lau_Number m = l_mathop(frexp)(x, &e);  /* 'x' fraction and exponent */
    ...
    if (m > 0) {  /* more digits? */
      buff[n++] = lau_getlocaledecpoint();  /* add radix point */
...
static int lau_number2strx (lau_State *L, char *buff, unsigned sz,
                            const char *fmt, lau_Number x) {
  ...
  if (fmt[SIZELENMOD] == 'A') {
    ...
    return lauL_error(L, "modifiers for format '%%a'/'%%A' not implemented");
  return n;
}
...
static void addquoted (lauL_Buffer *b, const char *s, size_t len) {
  lauL_addchar(b, '"');
  while (len--) {
    if (*s == '"' || *s == '\\' || *s == '\n') {
      lauL_addchar(b, '\\');
...
** scanned back by Lau. Use hexadecimal format for "common" numbers
** (to preserve precision); inf, -inf, and NaN are handled separately.
** (NaN cannot be expressed as a numeral, so we write '(0/0)' for it.)
*/
static int quotefloat (lau_State *L, char *buff, lau_Number n) {
  const char *s;  /* for the fixed representations */
  if (n == (lau_Number)HUGE_VAL)  /* inf? */
    s = "1e9999";
  else if (n == -(lau_Number)HUGE_VAL)  /* -inf? */
    s = "-1e9999";
  else if (n != n)  /* NaN? */
    s = "(0/0)";
  else {  /* format number as hexadecimal */
    int  nb = lau_number2strx(L, buff, MAX_ITEM,
                                 "%" LAU_NUMBER_FRMLEN "a", n);
    /* ensures that 'buff' string uses a dot as the radix character */
    if (memchr(buff, '.', cast_uint(nb)) == NULL) {  /* no dot? */
      char point = lau_getlocaledecpoint();  /* try locale point */
...
static void addliteral (lau_State *L, lauL_Buffer *b, int arg) {
  switch (lau_type(L, arg)) {
    case LAU_TSTRING: {
      size_t len;
      const char *s = lau_tolstring(L, arg, &len);
      addquoted(b, s, len);
    ...
    case LAU_TNUMBER: {
      char *buff = lauL_prepbuffsize(b, MAX_ITEM);
      int nb;
      if (!lau_isinteger(L, arg))  /* float? */
        nb = quotefloat(L, buff, lau_tonumber(L, arg));
      else {  /* integers */
        lau_Integer n = lau_tointeger(L, arg);
        const char *format = (n == LAU_MININTEGER)  /* corner case? */
                           ? "0x%" LAU_INTEGER_FRMLEN "x"  /* use hex */
                           : LAU_INTEGER_FMT;  /* else use default format */
        nb = l_sprintf(buff, MAX_ITEM, format, (LAUI_UACINT)n);
      }
      lauL_addsize(b, cast_uint(nb));
      break;
    }
    case LAU_TNIL: case LAU_TBOOLEAN: {
      lauL_tolstring(L, arg, NULL);
      lauL_addvalue(b);
      break;
    }
    default: {
      lauL_argerror(L, arg, "value has no literal form");
    }
  }
}
...
*/
static void checkformat (lau_State *L, const char *form, const char *flags,
                                       int precision) {
  ...
  if (!isalpha(cast_uchar(*spec)))  /* did not go to the end? */
    lauL_error(L, "invalid conversion specification: '%s'", form);
}
...
*/
static const char *getformat (lau_State *L, const char *strfrmt,
                                            char *form) {
  ...
  if (len >= MAX_FORMAT - 10)
    lauL_error(L, "invalid format (too long)");
  *(form++) = '%';
...
static int str_format (lau_State *L) {
  int top = lau_gettop(L);
  int arg = 1;
  size_t sfl;
  const char *strfrmt = lauL_checklstring(L, arg, &sfl);
  ...
  while (strfrmt < strfrmt_end) {
    if (*strfrmt != L_ESC)
      lauL_addchar(&b, *strfrmt++);
    else if (*++strfrmt == L_ESC)
      lauL_addchar(&b, *strfrmt++);  /* %% */
    else { /* format item */
      char form[MAX_FORMAT];  /* to store the format ('%...') */
      unsigned maxitem = MAX_ITEM;  /* maximum length for the result */
      char *buff = lauL_prepbuffsize(&b, maxitem);  /* to put result */
      int nb = 0;  /* number of bytes in result */
      if (++arg > top)
        return lauL_argerror(L, arg, "no value");
      strfrmt = getformat(L, strfrmt, form);
      switch (*strfrmt++) {
        case 'c': {
          checkformat(L, form, L_FMTFLAGSC, 0);
          nb = l_sprintf(buff, maxitem, form, (int)lauL_checkinteger(L, arg));
          break;
        }
        case 'd': case 'i':
          flags = L_FMTFLAGSI;
        ...
        case 'o': case 'x': case 'X':
          flags = L_FMTFLAGSX;
         intcase: {
          lau_Integer n = lauL_checkinteger(L, arg);
          checkformat(L, form, flags, 1);
          addlenmod(form, LAU_INTEGER_FRMLEN);
          nb = l_sprintf(buff, maxitem, form, (LAUI_UACINT)n);
          break;
        }
        case 'a': case 'A':
          checkformat(L, form, L_FMTFLAGSF, 1);
          addlenmod(form, LAU_NUMBER_FRMLEN);
          nb = lau_number2strx(L, buff, maxitem, form,
                                  lauL_checknumber(L, arg));
          break;
        case 'f':
          maxitem = MAX_ITEMF;  /* extra space for '%f' */
          buff = lauL_prepbuffsize(&b, maxitem);
          /* FALLTHROUGH */
        case 'e': case 'E': case 'g': case 'G': {
          lau_Number n = lauL_checknumber(L, arg);
          checkformat(L, form, L_FMTFLAGSF, 1);
          addlenmod(form, LAU_NUMBER_FRMLEN);
          nb = l_sprintf(buff, maxitem, form, (LAUI_UACNUMBER)n);
          break;
        }
        case 'p': {
          const void *p = lau_topointer(L, arg);
          checkformat(L, form, L_FMTFLAGSC, 0);
        ...
        case 'q': {
          if (form[2] != '\0')  /* modifiers? */
            return lauL_error(L, "specifier '%%q' cannot have modifiers");
          addliteral(L, &b, arg);
          break;
        }
        case 's': {
          size_t l;
          const char *s = lauL_tolstring(L, arg, &l);
          if (form[2] == '\0')  /* no modifiers? */
            lauL_addvalue(&b);  /* keep entire string */
          else {
            lauL_argcheck(L, l == strlen(s), arg, "string contains zeros");
            ...
            if (strchr(form, '.') == NULL && l >= 100) {
              ...
              lau_pop(L, 1);  /* remove result from 'lauL_tolstring' */
            }
          }
          break;
        }
        default: {  /* also treat cases 'pnLlh' */
          return lauL_error(L, "invalid conversion '%s' to 'format'", form);
        }
      }
      lau_assert(cast_uint(nb) < maxitem);
      lauL_addsize(&b, cast_uint(nb));
    }
  }
  lauL_pushresult(&b);
...
#if !defined(LAUL_PACKPADBYTE)
#define LAUL_PACKPADBYTE		0x00
#endif
...
/* size of a lau_Integer */
#define SZINT	((int)sizeof(lau_Integer))
...
typedef enum KOption {
  ...
  Knumber,	/* Lau "native" floating-point numbers */
...
static unsigned getnumlimit (Header *h, const char **fmt, size_t df) {
  ...
  if (l_unlikely((sz - 1u) >= MAXINTSIZE))
    return cast_uint(lauL_error(h->L,
...
** Initialize Header
*/
static void initheader (lau_State *L, Header *h) {
  h->L = L;
...
static KOption getoption (Header *h, const char **fmt, size_t *size) {
  ...
  struct cD { char c; union { LAUI_MAXALIGN; } u; };
  int opt = *((*fmt)++);
  *size = 0;  /* default */
  switch (opt) {
    ...
    case 'j': *size = sizeof(lau_Integer); return Kint;
    case 'J': *size = sizeof(lau_Integer); return Kuint;
    case 'T': *size = sizeof(size_t); return Kuint;
    case 'f': *size = sizeof(float); return Kfloat;
    case 'n': *size = sizeof(lau_Number); return Knumber;
    ...
    case 'c':
      *size = getnum(fmt, cast_sizet(-1));
      if (l_unlikely(*size == cast_sizet(-1)))
        lauL_error(h->L, "missing size for format option 'c'");
      return Kchar;
    case 'z': return Kzstr;
    ...
    default: lauL_error(h->L, "invalid format option '%c'", opt);
  }
  return Knop;
}
...
static KOption getdetails (Header *h, size_t totalsize, const char **fmt,
  ...
  if (opt == Kpaddalign) {  /* 'X' gets alignment from following option */
    if (**fmt == '\0' || getoption(h, fmt, &align) == Kchar || align == 0)
      lauL_argerror(h->L, 1, "invalid next option for option 'X'");
  }
    ...
    *ntoalign = 0;
    ...
    if (l_unlikely(!ispow2(align))) {  /* not a power of 2? */
      *ntoalign = 0;  /* to avoid warnings */
      lauL_argerror(h->L, 1, "format asks for alignment not power of 2");
    }
    else {
...
** the size of a Lau integer, correcting the extra sign-extension
** bytes if necessary (by default they would be zeros).
*/
static void packint (lauL_Buffer *b, lau_Unsigned n,
                     int islittle, unsigned size, int neg) {
  char *buff = lauL_prepbuffsize(b, size);
  unsigned i;
  ...
  lauL_addsize(b, size);  /* add result to buffer */
}
...
static int str_pack (lau_State *L) {
  lauL_Buffer b;
  Header h;
  const char *fmt = lauL_checkstring(L, 1);  /* format string */
  int arg = 1;  /* current argument to pack */
  size_t totalsize = 0;  /* accumulate total size of result */
  initheader(L, &h);
  lau_pushnil(L);  /* mark to separate arguments from string buffer */
  lauL_buffinit(L, &b);
  while (*fmt != '\0') {
    unsigned ntoalign;
    size_t size;
    KOption opt = getdetails(&h, totalsize, &fmt, &size, &ntoalign);
    lauL_argcheck(L, size + ntoalign <= MAX_SIZE - totalsize, arg,
                     "result too long");
    totalsize += ntoalign + size;
    while (ntoalign-- > 0)
     lauL_addchar(&b, LAUL_PACKPADBYTE);  /* fill alignment */
    arg++;
    switch (opt) {
      case Kint: {  /* signed integers */
        lau_Integer n = lauL_checkinteger(L, arg);
        if (size < SZINT) {  /* need overflow check? */
          lau_Integer lim = (lau_Integer)1 << ((size * NB) - 1);
          lauL_argcheck(L, -lim <= n && n < lim, arg, "integer overflow");
        }
        packint(&b, (lau_Unsigned)n, h.islittle, cast_uint(size), (n < 0));
        break;
      }
      case Kuint: {  /* unsigned integers */
        lau_Integer n = lauL_checkinteger(L, arg);
        if (size < SZINT)  /* need overflow check? */
          lauL_argcheck(L, (lau_Unsigned)n < ((lau_Unsigned)1 << (size * NB)),
                           arg, "unsigned overflow");
        packint(&b, (lau_Unsigned)n, h.islittle, cast_uint(size), 0);
        break;
      }
      case Kfloat: {  /* C float */
        float f = (float)lauL_checknumber(L, arg);  /* get argument */
        char *buff = lauL_prepbuffsize(&b, sizeof(f));
        /* move 'f' to final result, correcting endianness if needed */
        copywithendian(buff, (char *)&f, sizeof(f), h.islittle);
        lauL_addsize(&b, size);
        break;
      }
      case Knumber: {  /* Lau float */
        lau_Number f = lauL_checknumber(L, arg);  /* get argument */
        char *buff = lauL_prepbuffsize(&b, sizeof(f));
        /* move 'f' to final result, correcting endianness if needed */
        copywithendian(buff, (char *)&f, sizeof(f), h.islittle);
        lauL_addsize(&b, size);
      ...
      case Kdouble: {  /* C double */
        double f = (double)lauL_checknumber(L, arg);  /* get argument */
        char *buff = lauL_prepbuffsize(&b, sizeof(f));
        /* move 'f' to final result, correcting endianness if needed */
        copywithendian(buff, (char *)&f, sizeof(f), h.islittle);
        lauL_addsize(&b, size);
      ...
      case Kchar: {  /* fixed-size string */
        size_t len;
        const char *s = lauL_checklstring(L, arg, &len);
        lauL_argcheck(L, len <= size, arg, "string longer than given size");
        lauL_addlstring(&b, s, len);  /* add string */
        if (len < size) {  /* does it need padding? */
          size_t psize = size - len;  /* pad size */
          char *buff = lauL_prepbuffsize(&b, psize);
          memset(buff, LAUL_PACKPADBYTE, psize);
          lauL_addsize(&b, psize);
        }
        break;
      }
      case Kstring: {  /* strings with length count */
        size_t len;
        const char *s = lauL_checklstring(L, arg, &len);
        lauL_argcheck(L, size >= sizeof(lau_Unsigned) ||
                         len < ((lau_Unsigned)1 << (size * NB)),
                         arg, "string length does not fit in given size");
        /* pack length */
        packint(&b, (lau_Unsigned)len, h.islittle, cast_uint(size), 0);
        lauL_addlstring(&b, s, len);
        totalsize += len;
        break;
      }
      case Kzstr: {  /* zero-terminated string */
        size_t len;
        const char *s = lauL_checklstring(L, arg, &len);
        lauL_argcheck(L, strlen(s) == len, arg, "string contains zeros");
        lauL_addlstring(&b, s, len);
        lauL_addchar(&b, '\0');  /* add zero at the end */
        totalsize += len + 1;
        break;
      }
      case Kpadding: lauL_addchar(&b, LAUL_PACKPADBYTE);  /* FALLTHROUGH */
...
static int str_packsize (lau_State *L) {
  Header h;
  const char *fmt = lauL_checkstring(L, 1);  /* format string */
  size_t totalsize = 0;  /* accumulate total size of result */
  initheader(L, &h);
  while (*fmt != '\0') {
    ...
    size_t size;
    KOption opt = getdetails(&h, totalsize, &fmt, &size, &ntoalign);
    lauL_argcheck(L, opt != Kstring && opt != Kzstr, 1,
                     "variable-length format");
    size += ntoalign;  /* total space used by option */
    lauL_argcheck(L, totalsize <= MAX_SIZE - size,
                     1, "format result too large");
    totalsize += size;
  }
  lau_pushinteger(L, cast_st2S(totalsize));
  return 1;
}
...
** If size is smaller than the size of a Lau integer and integer
** is signed, must do sign extension (propagating the sign to the
** higher bits); if size is larger than the size of a Lau integer,
** it must check the unread bytes to see whether they do not cause an
** overflow.
*/
static lau_Integer unpackint (lau_State *L, const char *str,
  ...
  for (i = limit - 1; i >= 0; i--) {
    res <<= NB;
    res |= (lau_Unsigned)(unsigned char)str[islittle ? i : size - 1 - i];
  }
  if (size < SZINT) {  /* real size smaller than lau_Integer? */
    if (issigned) {  /* needs sign extension? */
      lau_Unsigned mask = (lau_Unsigned)1 << (size*NB - 1);
      res = ((res ^ mask) - mask);  /* do sign extension */
    }
  }
  else if (size > SZINT) {  /* must check unread bytes */
    int mask = (!issigned || (lau_Integer)res >= 0) ? 0 : MC;
    for (i = limit; i < size; i++) {
      if (l_unlikely((unsigned char)str[islittle ? i : size - 1 - i] != mask))
        lauL_error(L, "%d-byte integer does not fit into Lau Integer", size);
    }
  }
  return (lau_Integer)res;
}


static int str_unpack (lau_State *L) {
  Header h;
  const char *fmt = lauL_checkstring(L, 1);
  size_t ld;
  const char *data = lauL_checklstring(L, 2, &ld);
  size_t pos = posrelatI(lauL_optinteger(L, 3, 1), ld) - 1;
  int n = 0;  /* number of results */
  lauL_argcheck(L, pos <= ld, 3, "initial position out of string");
  initheader(L, &h);
  while (*fmt != '\0') {
    unsigned ntoalign;
    size_t size;
    KOption opt = getdetails(&h, pos, &fmt, &size, &ntoalign);
    lauL_argcheck(L, ntoalign + size <= ld - pos, 2,
                    "data string too short");
    pos += ntoalign;  /* skip alignment */
    /* stack space for item + next position */
    lauL_checkstack(L, 2, "too many results");
    n++;
    switch (opt) {
      case Kint:
      case Kuint: {
        lau_Integer res = unpackint(L, data + pos, h.islittle,
      ...
      case Kchar: {
        lau_pushlstring(L, data + pos, size);
        break;
      }
      case Kstring: {
        lau_Unsigned len = (lau_Unsigned)unpackint(L, data + pos,
                                          h.islittle, cast_int(size), 0);
        lauL_argcheck(L, len <= ld - pos - size, 2, "data string too short");
        lau_pushlstring(L, data + pos + size, cast_sizet(len));
        pos += cast_sizet(len);  /* skip string */
        break;
      }
      case Kzstr: {
        size_t len = strlen(data + pos);
        lauL_argcheck(L, pos + len < ld, 2,
                         "unfinished string for format 'z'");
        lau_pushlstring(L, data + pos, len);
  ...
  lau_pushinteger(L, cast_st2S(pos) + 1);  /* next position */
  return n + 1;
}
