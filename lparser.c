...
static l_noret error_expected (LexState *ls, int token) {
  lauX_syntaxerror(ls,
      lauO_pushfstring(ls->L, "%s expected", lauX_token2str(ls, token)));
}
...
static l_noret errorlimit (FuncState *fs, int limit, const char *what) {
  ...
  const char *where = (line == 0)
                      ? "main function"
                      : lauO_pushfstring(L, "function at line %d", line);
  msg = lauO_pushfstring(L, "too many %s (limit is %d) in %s",
                             what, limit, where);
  lauX_syntaxerror(fs->ls, msg);
}


void lauY_checklimit (FuncState *fs, int v, int l, const char *what) {
...
#define check_condition(ls,c,msg)	{ if (!(c)) lauX_syntaxerror(ls, msg); }
...
static void check_match (LexState *ls, int what, int who, int where) {
  if (l_unlikely(!testnext(ls, what))) {
    if (where == ls->linenumber)  /* all in the same line? */
      ...
      lauX_syntaxerror(ls, lauO_pushfstring(ls->L,
             "%s expected (to close %s at line %d)",
              lauX_token2str(ls, what), lauX_token2str(ls, who), where));
    }
  }
}
...
static short registerlocalvar (LexState *ls, FuncState *fs,
  ...
  lauM_growvector(ls->L, f->locvars, fs->ndebugvars, f->sizelocvars,
                  LocVar, SHRT_MAX, "local variables");
  while (oldsize < f->sizelocvars)
    f->locvars[oldsize++].varname = NULL;
  f->locvars[fs->ndebugvars].varname = varname;
  f->locvars[fs->ndebugvars].startpc = fs->pc;
  lauC_objbarrier(ls->L, f, varname);
...
static int new_varkind (LexState *ls, TString *name, lu_byte kind) {
  ...
  Dyndata *dyd = ls->dyd;
  Vardesc *var;
  lauM_growvector(L, dyd->actvar.arr, dyd->actvar.n + 1,
...
#define new_localvarliteral(ls,v) \
    new_localvar(ls,  \
      lauX_newstring(ls, "" v, (sizeof(v)/sizeof(char)) - 1));
...
** function.
*/
lu_byte lauY_nvarstack (FuncState *fs) {
...
static void check_readonly (LexState *ls, expdesc *e) {
  ...
  switch (e->k) {
    ...
    default:
      lau_assert(e->k == VINDEXI);  /* this one doesn't need any check */
      return;  /* integer index cannot be read-only */
  }
  if (varname)
    lauK_semerror(ls, "attempt to assign to const variable '%s'",
                      getstr(varname));
}
...
static void adjustlocalvars (LexState *ls, int nvars) {
  FuncState *fs = ls->fs;
  int reglevel = lauY_nvarstack(fs);
  int i;
  for (i = 0; i < nvars; i++) {
    int vidx = fs->nactvar++;
    ...
    lauY_checklimit(fs, reglevel, MAXVARS, "local variables");
  }
}
...
static Upvaldesc *allocupvalue (FuncState *fs) {
  ...
  lauY_checklimit(fs, fs->nups + 1, MAXUPVAL, "upvalues");
  lauM_growvector(fs->ls->L, f->upvalues, fs->nups, f->sizeupvalues,
...
static int newupvalue (FuncState *fs, TString *name, expdesc *v) {
  ...
  if (v->k == VLOCAL) {
    ...
    lau_assert(eqstr(name, getlocalvardesc(prev, v->u.var.vidx)->vd.name));
  }
  else {
    up->instack = 0;
    up->idx = cast_byte(v->u.info);
    up->kind = prev->f->upvalues[v->u.info].kind;
    lau_assert(eqstr(name, prev->f->upvalues[v->u.info].name));
  }
  up->name = name;
  lauC_objbarrier(fs->ls->L, fs->f, name);
  return fs->nups - 1;
}
  ...
  if (v >= 0) {  /* found? */
    if (!base) {
      if (var->k == VVARGVAR)  /* vararg parameter? */
        lauK_vapar2local(fs, var);  /* change it to a regular local */
      if (var->k == VLOCAL)
...
static void buildglobal (LexState *ls, TString *varname, expdesc *var) {
  ...
  if (var->k == VGLOBAL)
    lauK_semerror(ls, "%s is global when accessing variable '%s'",
                      LAU_ENV, getstr(varname));
  lauK_exp2anyregup(fs, var);  /* _ENV could be a constant */
  codestring(&key, varname);  /* key is variable name */
  lauK_indexed(fs, var, &key);  /* 'var' represents _ENV[varname] */
}
...
static void buildvar (LexState *ls, TString *varname, expdesc *var) {
  ...
  if (var->k == VGLOBAL) {  /* global name? */
    ...
    if (info == -2)
      lauK_semerror(ls, "variable '%s' not declared", getstr(varname));
    ...
    if (info != -1 && ls->dyd->actvar.arr[info].vd.kind == GDKCONST)
      ...
      lau_assert(info == -1 || ls->dyd->actvar.arr[info].vd.kind == GDKREG);
  }
}
...
static void adjust_assign (LexState *ls, int nvars, int nexps, expdesc *e) {
  ...
  lauK_checkstack(fs, needed);
  if (hasmultret(e->k)) {  /* last expression has multiple returns? */
    int extra = needed + 1;  /* discount last expression itself */
    if (extra < 0)
      extra = 0;
    lauK_setreturns(fs, e, extra);  /* last exp. provides the difference */
  }
  else {
    if (e->k != VVOID)  /* at least one expression? */
      lauK_exp2nextreg(fs, e);  /* close last expression */
    if (needed > 0)  /* missing values? */
      lauK_nil(fs, fs->freereg, needed);  /* complete with nils */
  }
  if (needed > 0)
    lauK_reserveregs(fs, needed);  /* registers for extra values */
  else  /* adding 'needed' is actually a subtraction */
    fs->freereg = cast_byte(fs->freereg + needed);  /* remove extra values */
}


#define enterlevel(ls)	lauE_incCstack(ls->L)
...
static void closegoto (LexState *ls, int g, Labeldesc *label, int bup) {
  ...
  lau_assert(eqstr(gt->name, label->name));
  ...
  lauK_patchlist(ls->fs, gt->pc, label->pc);  /* goto jumps to label */
...
static int newlabelentry (LexState *ls, Labellist *l, TString *name,
                          int line, int pc) {
  int n = l->n;
  lauM_growvector(ls->L, l->arr, n, l->size,
...
static int newgotoentry (LexState *ls, TString *name, int line) {
  FuncState *fs = ls->fs;
  int pc = lauK_jump(fs);  /* create jump */
  lauK_codeABC(fs, OP_CLOSE, 0, 1, 0);  /* spaceholder, marked as dead */
...
static void createlabel (LexState *ls, TString *name, int line, int last) {
  ...
  int l = newlabelentry(ls, ll, name, line, lauK_getlabel(fs));
...
static void enterblock (FuncState *fs, BlockCnt *bl, lu_byte isloop) {
  ...
  fs->bl = bl;
  lau_assert(fs->freereg == lauY_nvarstack(fs));
}
...
static l_noret undefgoto (LexState *ls, Labeldesc *gt) {
  ...
  lau_assert(!eqstr(gt->name, ls->brkn));
  lauK_semerror(ls, "no visible label '%s' for <goto> at line %d",
...
static void leaveblock (FuncState *fs) {
  ...
  if (bl->previous && bl->upval)  /* need a 'close'? */
    lauK_codeABC(fs, OP_CLOSE, stklevel, 0, 0);
  fs->freereg = stklevel;  /* free registers */
  removevars(fs, bl->nactvar);  /* remove block locals */
  lau_assert(bl->nactvar == fs->nactvar);  /* back to level on entry */
...
static Proto *addprototype (LexState *ls) {
  ...
  if (fs->np >= f->sizep) {
    int oldsize = f->sizep;
    lauM_growvector(L, f->p, fs->np, f->sizep, Proto *, MAXARG_Bx, "functions");
    while (oldsize < f->sizep)
      f->p[oldsize++] = NULL;
  }
  f->p[fs->np++] = clp = lauF_newproto(L);
  lauC_objbarrier(L, f, clp);
  return clp;
}
...
static void codeclosure (LexState *ls, expdesc *v) {
  ...
  init_exp(v, VRELOC, lauK_codeABx(fs, OP_CLOSURE, 0, fs->np - 1));
  lauK_exp2nextreg(fs, v);  /* fix it at the last register */
}
...
static void open_func (LexState *ls, FuncState *fs, BlockCnt *bl) {
  ...
  fs->bl = NULL;
  f->source = ls->source;
  lauC_objbarrier(L, f, f->source);
  f->maxstacksize = 2;  /* registers 0/1 are always valid */
  fs->kcache = lauH_new(L);  /* create table for function */
  lauD_anchorobj(L, ls->h, obj2gco(fs->kcache));  /* anchor it */
  enterblock(fs, bl, 0);
}
...
static void close_func (LexState *ls) {
  ...
  Proto *f = fs->f;
  TValue temp;
  lauK_ret(fs, lauY_nvarstack(fs), 0);  /* final return */
  leaveblock(fs);
  lau_assert(fs->bl == NULL);
  lauK_finish(fs);
  lauM_shrinkvector(L, f->code, f->sizecode, fs->pc, Instruction);
  lauM_shrinkvector(L, f->lineinfo, f->sizelineinfo, fs->pc, ls_byte);
  lauM_shrinkvector(L, f->abslineinfo, f->sizeabslineinfo,
                       fs->nabslineinfo, AbsLineInfo);
  lauM_shrinkvector(L, f->k, f->sizek, fs->nk, TValue);
  lauM_shrinkvector(L, f->p, f->sizep, fs->np, Proto *);
  lauM_shrinkvector(L, f->locvars, f->sizelocvars, fs->ndebugvars, LocVar);
  lauM_shrinkvector(L, f->upvalues, f->sizeupvalues, fs->nups, Upvaldesc);
  /* remove kcache table from scanner table ("weigh" its anchor) */
  sethvalue(L, &temp, fs->kcache);  /* key to be set to nil */
  lauH_set(L, ls->h, &temp, &G(L)->nilvalue);
  ls->fs = fs->prev;
  lauC_checkGC(L);
}
...
static void fieldsel (LexState *ls, expdesc *v) {
  ...
  FuncState *fs = ls->fs;
  expdesc key;
  lauK_exp2anyregup(fs, v);
  lauX_next(ls);  /* skip the dot or colon */
  codename(ls, &key);
  lauK_indexed(fs, v, &key);
}


static void yindex (LexState *ls, expdesc *v) {
  /* index -> '[' expr ']' */
  lauX_next(ls);  /* skip the '[' */
  expr(ls, v);
  lauK_exp2val(ls->fs, v);
  checknext(ls, ']');
}
...
static void recfield (LexState *ls, ConsControl *cc) {
  ...
  checknext(ls, '=');
  tab = *cc->t;
  lauK_indexed(fs, &tab, &key);
  expr(ls, &val);
  lauK_storevar(fs, &tab, &val);
  fs->freereg = reg;  /* free registers */
}


static void closelistfield (FuncState *fs, ConsControl *cc) {
  lau_assert(cc->tostore > 0);
  lauK_exp2nextreg(fs, &cc->v);
  cc->v.k = VVOID;
  if (cc->tostore >= cc->maxtostore) {
    lauK_setlist(fs, cc->t->u.info, cc->na, cc->tostore);  /* flush */
    cc->na += cc->tostore;
...
static void lastlistfield (FuncState *fs, ConsControl *cc) {
  ...
  if (hasmultret(cc->v.k)) {
    lauK_setmultret(fs, &cc->v);
    lauK_setlist(fs, cc->t->u.info, cc->na, LAU_MULTRET);
    cc->na--;  /* do not count last expression (unknown number of elements) */
  }
  else {
    if (cc->v.k != VVOID)
      lauK_exp2nextreg(fs, &cc->v);
    lauK_setlist(fs, cc->t->u.info, cc->na, cc->tostore);
  }
  cc->na += cc->tostore;
}
...
static void field (LexState *ls, ConsControl *cc) {
  ...
  switch(ls->t.token) {
    case TK_NAME: {  /* may be 'listfield' or 'recfield' */
      if (lauX_lookahead(ls) != '=')  /* expression? */
        listfield(ls, cc);
      else
        recfield(ls, cc);
      break;
...
static void parlist (LexState *ls) {
  ...
  lauK_reserveregs(fs, fs->nactvar);
}
...
static void funcargs (LexState *ls, expdesc *f) {
  ...
  switch (ls->t.token) {
    ...
    case TK_STRING: {  /* funcargs -> STRING */
      ...
      lauX_next(ls);  /* must use 'seminfo' before 'next' */
      break;
    }
    default: {
      lauX_syntaxerror(ls, "function arguments expected");
      return;  /* to avoid warnings */
    }
  }
  lau_assert(f->k == VNONRELOC);
  base = f->u.info;  /* base register for call */
  if (hasmultret(args.k))
    nparams = LAU_MULTRET;  /* open call */
  else {
    if (args.k != VVOID)
      lauK_exp2nextreg(fs, &args);  /* close last argument */
    nparams = fs->freereg - (base+1);
  }
  init_exp(f, VCALL, lauK_codeABC(fs, OP_CALL, base, nparams+1, 2));
...
static void primaryexp (LexState *ls, expdesc *v) {
  ...
  switch (ls->t.token) {
    ...
    default: {
      lauX_syntaxerror(ls, "unexpected symbol");
    }
  }
}
...
static BinOpr subexpr (LexState *ls, expdesc *v, int limit) {
  ...
  if (uop != OPR_NOUNOPR) {  /* prefix (unary) operator? */
    int line = ls->linenumber;
    lauX_next(ls);  /* skip operator */
    subexpr(ls, v, UNARY_PRIORITY);
    lauK_prefix(ls->fs, uop, v, line);
  }
  else simpleexp(ls, v);
  ...
  while (op != OPR_NOBINOPR && priority[op].left > limit) {
    ...
    int line = ls->linenumber;
    lauX_next(ls);  /* skip operator */
    lauK_infix(ls->fs, op, v);
    /* read sub-expression with higher priority */
    nextop = subexpr(ls, &v2, priority[op].right);
    lauK_posfix(ls->fs, op, v, &v2, line);
    op = nextop;
  }
  leavelevel(ls);
...
static void check_conflict (LexState *ls, struct LHS_assign *lh, expdesc *v) {
  ...
  if (conflict) {
    ...
    if (v->k == VLOCAL)
      lauK_codeABC(fs, OP_MOVE, extra, v->u.var.ridx, 0);
    else
      lauK_codeABC(fs, OP_GETUPVAL, extra, v->u.info, 0);
    lauK_reserveregs(fs, 1);
  }
}
...
static void storevartop (FuncState *fs, expdesc *var) {
  ...
  lauK_storevar(fs, var, &e);  /* will also free the top register */
}
...
static void restassign (LexState *ls, struct LHS_assign *lh, int nvars) {
  ...
  if (testnext(ls, ',')) {  /* restassign -> ',' suffixedexp restassign */
    ...
    if (nexps != nvars)
      ...
      lauK_setoneret(ls->fs, &e);  /* close last expression */
      lauK_storevar(ls->fs, &lh->v, &e);
...
static void breakstat (LexState *ls, int line) {
  ...
  lauX_syntaxerror(ls, "break outside loop");
 ok:
...
static void exp1 (LexState *ls) {
  expdesc e;
  expr(ls, &e);
  lauK_exp2nextreg(ls->fs, &e);
  lau_assert(e.k == VNONRELOC);
}


/*
** Fix for instruction at position 'pc' to jump to 'dest'.
** (Jump addresses are relative in Lau). 'back' true means
** a back jump.
*/
...
static void forbody (LexState *ls, int base, int line, int nvars, int isgen) {
  ...
  if (isgen) {  /* generic for? */
    lauK_codeABC(fs, OP_TFORCALL, base, 0, nvars);
    lauK_fixline(fs, line);
  }
  endfor = lauK_codeABx(fs, forloop[isgen], base, 0);
...
static void forlist (LexState *ls, TString *indexname) {
  ...
  lauK_checkstack(fs, 2);  /* extra space to call iterator */
...
static void ifstat (LexState *ls, int line) {
  ...
  lauK_patchtohere(fs, escapelist);  /* patch escape list to 'if' end */
}
...
static lu_byte getvarattribute (LexState *ls, lu_byte df) {
  ...
  if (testnext(ls, '<')) {
    ...
    if (strcmp(attr, "const") == 0)
      ...
      lauK_semerror(ls, "unknown attribute '%s'", attr);
  }
...
static void checktoclose (FuncState *fs, int level) {
  if (level != -1) {  /* is there a to-be-closed variable? */
    marktobeclosed(fs);
    lauK_codeABC(fs, OP_TBC, reglevel(fs, level), 0, 0);
  }
}
...
static void localstat (LexState *ls) {
  ...
  do {  /* for each variable */
    ...
    if (kind == RDKTOCLOSE) {  /* to-be-closed? */
      if (toclose != -1)  /* one already present? */
        lauK_semerror(ls, "multiple to-be-closed variables in local list");
  ...
  if (nvars == nexps &&  /* no adjustments? */
      ...
      lauK_exp2const(fs, &e, &var->k)) {  /* compile-time constant? */
...
static void funcstat (LexState *ls, int line) {
  ...
  int ismethod;
  expdesc v, b;
  lauX_next(ls);  /* skip FUNCTION */
  ismethod = funcname(ls, &v);
  check_readonly(ls, &v);
  body(ls, &b, ismethod, line);
  lauK_storevar(ls->fs, &v, &b);
  lauK_fixline(ls->fs, line);  /* definition "happens" in the first line */
}
...
static void retstat (LexState *ls) {
  ...
  if (block_follow(ls, 1))
    ...
    if (hasmultret(e.k)) {
      lauK_setmultret(fs, &e);
      if (e.k == VCALL && nret == 1 && !fs->bl->insidetbc) {  /* tail call? */
        SET_OPCODE(getinstruction(fs,&e), OP_TAILCALL);
        lau_assert(GETARG_A(getinstruction(fs,&e)) == lauY_nvarstack(fs));
      }
      nret = LAU_MULTRET;  /* return all values */
    }
    else {
      if (nret == 1)  /* only one single value? */
        first = lauK_exp2anyreg(fs, &e);  /* can use original slot */
      else {  /* values must go to the top of the stack */
        lauK_exp2nextreg(fs, &e);
        lau_assert(nret == fs->freereg - first);
      }
    }
  }
...
static void statement (LexState *ls) {
  ...
  lau_assert(ls->fs->f->maxstacksize >= ls->fs->freereg &&
             ls->fs->freereg >= lauY_nvarstack(ls->fs));
  ls->fs->freereg = lauY_nvarstack(ls->fs);  /* free registers */
  leavelevel(ls);
}
...
static void mainfunc (LexState *ls, FuncState *fs) {
  ...
  env->kind = VDKREG;
  env->name = ls->envn;
  lauC_objbarrier(ls->L, fs->f, env->name);
  lauX_next(ls);  /* read first token */
...
LClosure *lauY_parser (lau_State *L, ZIO *z, Table *anchor, Mbuffer *buff,
                       Dyndata *dyd, const char *name, int firstchar) {
  LexState lexstate;
  FuncState funcstate;
  LClosure *cl;
  lexstate.h = anchor;  /* table for scanner */
  cl = lauF_newLclosure(L, 1);  /* create main closure */
  lauD_anchorobj(L, anchor, obj2gco(cl));  /* anchor it in scanner table */
  funcstate.f = cl->p = lauF_newproto(L);
  lauC_objbarrier(L, cl, cl->p);
  funcstate.f->source = lauS_new(L, name);  /* create and anchor TString */
  lauC_objbarrier(L, funcstate.f, funcstate.f->source);
  lexstate.buff = buff;
  lexstate.dyd = dyd;
  dyd->actvar.n = dyd->gt.n = dyd->label.n = 0;
  lauX_setinput(L, &lexstate, z, funcstate.f->source, firstchar);
  mainfunc(&lexstate, &funcstate);
  lau_assert(!funcstate.prev && funcstate.nups == 1 && !lexstate.fs);
  /* all scopes should be correctly finished */
  lau_assert(dyd->actvar.n == 0 && dyd->gt.n == 0 && dyd->label.n == 0);
  return cl;
}
