...
#define lauC_white(g)	cast_byte((g)->currentwhite & WHITEBITS)
...
/*
** Minor collections will shift to major ones after LAUI_MINORMAJOR%
** bytes become old.
*/
#define LAUI_MINORMAJOR         70

/*
** Major collections will shift to minor ones after a collection
** collects at least LAUI_MAJORMINOR% of the new bytes.
*/
#define LAUI_MAJORMINOR         50

/*
** A young (minor) collection will run after creating LAUI_GENMINORMUL%
** new bytes.
*/
#define LAUI_GENMINORMUL         20


/* incremental */

/* Number of bytes must be LAUI_GCPAUSE% before starting new cycle */
#define LAUI_GCPAUSE    250

/*
** Step multiplier: The collector handles LAUI_GCMUL% work units for
** each new allocated word. (Each "work unit" corresponds roughly to
** sweeping one object or traversing one slot.)
*/
#define LAUI_GCMUL      200

/* How many bytes to allocate before next GC step */
#define LAUI_GCSTEPSIZE	(200 * sizeof(Table))


#define setgcparam(g,p,v)  (g->gcparams[LAU_GCP##p] = lauO_codeparam(v))
#define applygcparam(g,p,x)  lauO_applyparam(g->gcparams[LAU_GCP##p], x)
...
#define GCSTPCLS	4  /* bit true when closing Lau state */
...
#define condchangemem(L,pre,pos,emg)  \
	{ if (gcrunning(G(L))) { pre; lauC_fullgc(L, emg); pos; } }
#endif

#define lauC_condGC(L,pre,pos) \
	{ if (G(L)->GCdebt <= 0) { pre; lauC_step(L); pos;}; \
	  condchangemem(L,pre,pos,0); }

/* more often than not, 'pre'/'pos' are empty */
#define lauC_checkGC(L)		lauC_condGC(L,(void)0,(void)0)


#define lauC_objbarrier(L,p,o) (  \
	(isblack(p) && iswhite(o)) ? \
	lauC_barrier_(L,obj2gco(p),obj2gco(o)) : cast_void(0))

#define lauC_barrier(L,p,v) (  \
	iscollectable(v) ? lauC_objbarrier(L,p,gcvalue(v)) : cast_void(0))

#define lauC_objbarrierback(L,p,o) (  \
	(isblack(p) && iswhite(o)) ? lauC_barrierback_(L,p) : cast_void(0))

#define lauC_barrierback(L,p,v) (  \
	iscollectable(v) ? lauC_objbarrierback(L, p, gcvalue(v)) : cast_void(0))

LAUI_FUNC void lauC_fix (lau_State *L, GCObject *o);
LAUI_FUNC void lauC_freeallobjects (lau_State *L);
LAUI_FUNC void lauC_step (lau_State *L);
LAUI_FUNC void lauC_runtilstate (lau_State *L, int state, int fast);
LAUI_FUNC void lauC_fullgc (lau_State *L, int isemergency);
LAUI_FUNC GCObject *lauC_newobj (lau_State *L, lu_byte tt, size_t sz);
LAUI_FUNC GCObject *lauC_newobjdt (lau_State *L, lu_byte tt, size_t sz,
                                                 size_t offset);
LAUI_FUNC void lauC_barrier_ (lau_State *L, GCObject *o, GCObject *v);
LAUI_FUNC void lauC_barrierback_ (lau_State *L, GCObject *o);
LAUI_FUNC void lauC_checkfinalizer (lau_State *L, GCObject *o, Table *mt);
LAUI_FUNC void lauC_changemode (lau_State *L, int newmode);
