...
** it fits in a 'size_t'.
*/
#define MAXHSIZE	lauM_limitN(1 << MAXHBITS, Node)
...
static const Node dummynode_ = {
  {{NULL}, LAU_VEMPTY,  /* value's value and type */
   LAU_TDEADKEY, 0, {NULL}}  /* key type, next, and key value */
};
...
*/
static Node *hashint (const Table *t, lau_Integer i) {
  lau_Unsigned ui = l_castS2U(i);
...
static unsigned l_hashfloat (lau_Number n) {
  int i;
  lau_Integer ni;
  n = l_mathop(frexp)(n, &i) * -cast_num(INT_MIN);
  if (!lau_numbertointeger(n, &ni)) {  /* is 'n' inf/-inf/NaN? */
    lau_assert(laui_numisnan(n) || l_mathop(fabs)(n) == cast_num(HUGE_VAL));
    return 0;
  }
  else {  /* normal case */
...
static Node *mainpositionTV (const Table *t, const TValue *key) {
  switch (ttypetag(key)) {
    case LAU_VNUMINT: {
      lau_Integer i = ivalue(key);
      return hashint(t, i);
    }
    case LAU_VNUMFLT: {
      lau_Number n = fltvalue(key);
    ...
    case LAU_VLNGSTR: {
      TString *ts = tsvalue(key);
      return hashpow2(t, lauS_hashlongstr(ts));
    }
    case LAU_VFALSE:
...
l_sinline Node *mainpositionfromnode (const Table *t, Node *nd) {
  TValue key;
  getnodekey(cast(lau_State *, NULL), &key, nd);
...
static int equalkey (const TValue *k1, const Node *n2, int deadok) {
  if (rawtt(k1) != keytt(n2)) {  /* not the same variants? */
    if (keyisshrstr(n2) && ttislngstring(k1)) {
      ...
      return lauS_eqstr(tsvalue(k1), keystrval(n2));
    }
    ...
    switch (keytt(n2)) {
      ...
      case LAU_VNUMFLT:
        return laui_numeq(fltvalue(k1), fltvalueraw(keyval(n2)));
      case LAU_VLIGHTUSERDATA:
        return pvalue(k1) == pvalueraw(keyval(n2));
      case LAU_VLCF:
        return fvalue(k1) == fvalueraw(keyval(n2));
      case ctb(LAU_VLNGSTR):
        return lauS_eqstr(tsvalue(k1), keystrval(n2));
      default:
        return gcvalue(k1) == gcvalueraw(keyval(n2));
    }
...
** the range [1, limit].
*/
static unsigned checkrange (lau_Integer k, unsigned limit) {
...
*/
static unsigned findindex (lau_State *L, Table *t, TValue *key,
                               unsigned asize) {
  unsigned int i;
  ...
  if (i != 0)  /* is 'key' inside array part? */
    ...
    if (l_unlikely(isabstkey(n)))
      lauG_runerror(L, "invalid key to 'next'");  /* key not found */
...
int lauH_next (lau_State *L, Table *t, StkId key) {
...
** entry: Two values plus 'next' versus one value.) Evalaute with size_t
** to avoid overflows.
*/
...
static void countint (lau_Integer key, Counters *ct) {
  unsigned int k = arrayindex(key);
  if (k != 0) {  /* is 'key' an array index? */
    ct->nums[lauO_ceillog2(k)]++;  /* count as such */
    ct->na++;
  }
}
...
static void numusehash (const Table *t, Counters *ct) {
  ...
  while (i--) {
    Node *n = &t->node[i];
    if (isempty(gval(n))) {
      lau_assert(!keyisnil(n));  /* entry was deleted; key cannot be nil */
      ct->deleted = 1;
    }
    else {
      total++;
...
*/
static Value *resizearray (lau_State *L , Table *t,
                               unsigned oldasize,
                               unsigned newasize) {
  if (oldasize == newasize)
    ...
    lauM_freemem(L, op, concretesize(oldasize));  /* free it */
    return NULL;
  }
  else {
    size_t newasizeb = concretesize(newasize);
    Value *np = cast(Value *,
                  lauM_reallocvector(L, NULL, 0, newasizeb, lu_byte));
    ...
    if (oldasize > 0) {
      ...
      lau_assert(tomoveb > 0);
      memcpy(np - tomove, op - tomove, tomoveb);
      lauM_freemem(L, op - oldasize, oldasizeb);  /* free old block */
    }
    return np;
  }
}
...
** overflow.
*/
static void setnodevector (lau_State *L, Table *t, unsigned size) {
  if (size == 0) {  /* no elements to hash part? */
    ...
    int i;
    int lsize = lauO_ceillog2(size);
    if (lsize > MAXHBITS || (1 << lsize) > MAXHSIZE)
      lauG_runerror(L, "table overflow");
    size = twoto(lsize);
    if (lsize < LIMFORLAST)  /* no 'lastfree' field? */
      t->node = lauM_newvector(L, size, Node);
    else {
      size_t bsize = size * sizeof(Node) + sizeof(Limbox);
      char *node = lauM_newblock(L, bsize);
...
*/
static void reinserthash (lau_State *L, Table *ot, Table *t) {
  unsigned j;
...
static void clearNewSlice (Table *t, unsigned oldasize, unsigned newasize) {
  for (; oldasize < newasize; oldasize++)
    *getArrTag(t, oldasize) = LAU_VEMPTY;
}
...
** part.
*/
void lauH_resize (lau_State *L, Table *t, unsigned newasize,
                                          unsigned nhsize) {
  ...
  if (l_unlikely(newarray == NULL && newasize > 0)) {  /* allocation failed? */
    ...
    lauM_error(L);  /* raise error (with array unchanged) */
  }
...
void lauH_resizearray (lau_State *L, Table *t, unsigned int nasize) {
  unsigned nsize = allocsizenode(t);
  lauH_resize(L, t, nasize, nsize);
}
...
*/
static void rehash (lau_State *L, Table *t, const TValue *ek) {
...
Table *lauH_new (lau_State *L) {
  GCObject *o = lauC_newobj(L, LAU_VTABLE, sizeof(Table));
  Table *t = gco2t(o);
  t->metatable = NULL;
...
** Frees a table.
*/
void lauH_free (lau_State *L, Table *t) {
  freehash(L, t);
...
static int insertkey (Table *t, const TValue *key, TValue *value) {
  ...
  lau_assert(isabstkey(getgeneric(t, key, 0)));
  if (!isempty(gval(mp)) || isdummy(t)) {  /* main position is taken? */
    Node *othern;
    Node *f = getfreepos(t);  /* get a free place */
    if (f == NULL)  /* cannot find a free place? */
      return 0;
    lau_assert(!isdummy(t));
    ...
    if (othern != mp) {  /* is colliding node out of its main position? */
      ...
      *f = *mp;  /* copy colliding node into free pos. (mp->next also goes) */
      ...
      else lau_assert(gnext(f) == 0);
      gnext(mp) = cast_int(f - mp);
      mp = f;
    }
  }
  setnodekey(mp, key);
  lau_assert(isempty(gval(mp)));
  setobj2t(cast(lau_State *, 0), gval(mp), value);
  return 1;
}
...
static void newcheckedkey (Table *t, const TValue *key, TValue *value) {
  ...
  if (i > 0)  /* is key in the array part? */
    ...
    lau_assert(done);  /* it cannot fail */
    cast(void, done);  /* to avoid warnings */
  }
}


static void lauH_newkey (lau_State *L, Table *t, const TValue *key,
                                                 TValue *value) {
  if (!ttisnil(value)) {  /* do not insert nil values */
    ...
    lauC_barrierback(L, obj2gco(t), key);
...
static const TValue *getintfromhash (Table *t, lau_Integer key) {
  Node *n = hashint(t, key);
  lau_assert(!ikeyinarray(t, key));
  for (;;) {  /* check whether 'key' is somewhere in the chain */
...
static int hashkeyisempty (Table *t, lau_Unsigned key) {
...
static lu_byte finishnodeget (const TValue *val, TValue *res) {
  if (!ttisnil(val)) {
    setobj(((lau_State*)NULL), res, val);
  }
  return ttypetag(val);
}


lu_byte lauH_getint (Table *t, lau_Integer key, TValue *res) {
...
*/
const TValue *lauH_Hgetshortstr (Table *t, TString *key) {
  Node *n = hashstr(t, key);
  lau_assert(strisshr(key));
  for (;;) {  /* check whether 'key' is somewhere in the chain */
...
lu_byte lauH_getshortstr (Table *t, TString *key, TValue *res) {
  return finishnodeget(lauH_Hgetshortstr(t, key), res);
}
...
static const TValue *Hgetlongstr (Table *t, TString *key) {
  TValue ko;
  lau_assert(!strisshr(key));
  setsvalue(cast(lau_State *, NULL), &ko, key);
...
static const TValue *Hgetstr (Table *t, TString *key) {
  if (strisshr(key))
    return lauH_Hgetshortstr(t, key);
  else
    return Hgetlongstr(t, key);
}


lu_byte lauH_getstr (Table *t, TString *key, TValue *res) {
...
*/
lu_byte lauH_get (Table *t, const TValue *key, TValue *res) {
  const TValue *slot;
  switch (ttypetag(key)) {
    case LAU_VSHRSTR:
      slot = lauH_Hgetshortstr(t, tsvalue(key));
      break;
    case LAU_VNUMINT:
      return lauH_getint(t, ivalue(key), res);
    case LAU_VNIL:
      slot = &absentkey;
      break;
    case LAU_VNUMFLT: {
      lau_Integer k;
      if (lauV_flttointeger(fltvalue(key), &k, F2Ieq)) /* integral index? */
        return lauH_getint(t, k, res);  /* use specialized version */
      /* else... */
    }  /* FALLTHROUGH */
    default:
...
** of its result, to be used by 'lauH_finishset'.
*/
...
static int finishnodeset (Table *t, const TValue *slot, TValue *val) {
  if (!ttisnil(slot)) {
    setobj(((lau_State*)NULL), cast(TValue*, slot), val);
    return HOK;  /* success */
  }
  else
...
int lauH_psetint (Table *t, lau_Integer key, TValue *val) {
  lau_assert(!ikeyinarray(t, key));
  return finishnodeset(t, getintfromhash(t, key), val);
}


static int psetint (Table *t, lau_Integer key, TValue *val) {
  int hres;
  lauH_fastseti(t, key, val, hres);
  return hres;
}
...
**    return finishnodeset(t, lauH_Hgetshortstr(t, key), val);
...
int lauH_psetshortstr (Table *t, TString *key, TValue *val) {
  const TValue *slot = lauH_Hgetshortstr(t, key);
  if (!ttisnil(slot)) {  /* key already has a value? (all too common) */
    setobj(((lau_State*)NULL), cast(TValue*, slot), val);  /* update it */
    return HOK;  /* done */
  }
    ...
    if (isabstkey(slot) &&  /* key is absent? */
      ...
      setsvalue(cast(lau_State *, NULL), &tk, key);
...
int lauH_psetstr (Table *t, TString *key, TValue *val) {
  if (strisshr(key))
    return lauH_psetshortstr(t, key, val);
  else
    return finishnodeset(t, Hgetlongstr(t, key), val);
}


int lauH_pset (Table *t, const TValue *key, TValue *val) {
  switch (ttypetag(key)) {
    case LAU_VSHRSTR: return lauH_psetshortstr(t, tsvalue(key), val);
    case LAU_VNUMINT: return psetint(t, ivalue(key), val);
    case LAU_VNIL: return HNOTFOUND;
    case LAU_VNUMFLT: {
      lau_Integer k;
      if (lauV_flttointeger(fltvalue(key), &k, F2Ieq)) /* integral index? */
...
*/
void lauH_finishset (lau_State *L, Table *t, const TValue *key,
  ...
  if (hres == HNOTFOUND) {
    ...
    if (l_unlikely(ttisnil(key)))
      lauG_runerror(L, "table index is nil");
    else if (ttisfloat(key)) {
      lau_Number f = fltvalue(key);
      lau_Integer k;
      if (lauV_flttointeger(f, &k, F2Ieq)) {  /* is key equal to an integer? */
        setivalue(&aux, k);
        actk = &aux;  /* use the integer as the key */
      }
      else if (l_unlikely(laui_numisnan(f)))
        lauG_runerror(L, "table index is NaN");
    }
    else if (isextstr(key)) {  /* external string? */
      /* If string is short, must internalize it to be used as table key */
      TString *ts = lauS_normstr(L, tsvalue(key));
      setsvalue2s(L, L->top.p++, ts);  /* anchor 'ts' (EXTRA_STACK) */
      lauH_newkey(L, t, s2v(L->top.p - 1), value);
      L->top.p--;
      return;
    }
    lauH_newkey(L, t, actk, value);
  }
...
*/
void lauH_set (lau_State *L, Table *t, const TValue *key, TValue *value) {
  int hres = lauH_pset(t, key, value);
  if (hres != HOK)
    lauH_finishset(L, t, key, value, hres);
}
...
*/
void lauH_setint (lau_State *L, Table *t, lau_Integer key, TValue *value) {
...
** LAU_MAXINTEGER. If it is absent, we are ready for the binary search.
...
** width.
*/
static lau_Unsigned hash_search (lau_State *L, Table *t, unsigned asize) {
  lau_Unsigned i = asize + 1;  /* caller ensures t[i] is present */
  unsigned rnd = G(L)->seed;
  int n = (asize > 0) ? lauO_ceillog2(asize) : 0;  /* width of 'asize' */
  unsigned mask = (1u << n) - 1;  /* 11...111 with the width of 'asize' */
  unsigned incr = (rnd & mask) + 1;  /* first increment (at least 1) */
  lau_Unsigned j = (incr <= l_castS2U(LAU_MAXINTEGER) - i) ? i + incr : i + 1;
  rnd >>= n;  /* used 'n' bits from 'rnd' */
  while (!hashkeyisempty(t, j)) {  /* repeat until an absent t[j] */
    i = j;  /* 'i' is a present index */
    if (j <= l_castS2U(LAU_MAXINTEGER)/2 - 1) {
...
static lau_Unsigned newhint (Table *t, unsigned hint) {
  lau_assert(hint <= t->asize);
  *lenhint(t) = hint;
  return hint;
}
...
*/
lau_Unsigned lauH_getn (lau_State *L, Table *t) {
  ...
  lau_assert(asize == 0 || !arraykeyisempty(t, asize));
...
Node *lauH_mainposition (const Table *t, const TValue *key) {
  return mainpositionTV(t, key);
}
